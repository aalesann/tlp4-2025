...
> **Objetivo:** Implementar una funcionalidad *muy simple* con enfoque TDD (primero el test, luego el código).
...
describe('TodoService.stats()', () => {
  it('devuelve totales correctos (total, completed, pending)', () => {
    ...
    // assert
    expect(s).toEqual({ total: 3, completed: 1, pending: 2 });
  });
});
```
...
npm run test:watch     # re-ejecuta al guardar archivos
